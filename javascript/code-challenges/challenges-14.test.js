...
const screenForNames = (arr) => {
  ...
  return arr.filter(element => {
    return /^(Mr||Dr||Mrs||Ms).\s[A-Z]/.test(element);
  })
}
...
const toTitleCase = (arr) => {
  ...
  return arr.map(element => {
    return element.replace(/^\w/, element[0].toUpperCase());;
  })
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const names = arr.reduce((acc, value) => {
...
//the commented function is for 3x3 size board only.
const detectTicTacToeWin = (board) => {
  ...
  if (tic[0] == tic[1] && tic[0] == tic[2] && tic[0] != '') {
    return true;
  } else if (tic[0] == tic[1] && tic[0] == tic[2] && tic[0] != '') {
    return true;
  } else if (tic[3] == tic[4] && tic[3] == tic[5] && tic[3] != '') {
    return true;
  } else if (tic[6] == tic[7] && tic[6] == tic[8] && tic[6] != '') {
    return true;
  } else if (tic[0] == tic[3] && tic[0] == tic[6] && tic[0] != '') {
    return true;
  } else if (tic[1] == tic[4] && tic[1] == tic[7] && tic[1] != '') {
    return true;
  } else if (tic[0] == tic[4] && tic[0] == tic[8] && tic[0] != '') {
    return true;
  } else if (tic[2] == tic[4] && tic[2] == tic[6] && tic[0] != '') {
    return true;
  } else {
    return false;
  }
